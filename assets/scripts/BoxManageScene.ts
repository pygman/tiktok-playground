...
export class BoxManage extends Component {
  ...
  onLoad() {
    ...
    this.node.getChildByName("BuyBag").on(Input.EventType.TOUCH_END, (_) => {
      ...
      this.client.purchase_box().then((resp) => {
        console.log("tx hash: ", resp);
        ...
        msgBoxNode
          ...
          .getComponent(
            cc.Label
          ).string = `Do Buy Bag ${this.selected} success hash:\n${resp}`;
    ...
    this.client.get_boxes().then((perosnalItems) => {
      ...
      this.boxes.map((box, i) => {
        ...
        boxNode.setPosition(
          (560 * 2 * (i + 1)) / (this.boxes.length + 1) - 560,
          20
        );
    ...
    const bagClick = (i: number) => {
      if (this.selected !== -1) {
        this.node
          .getChildByName(`Box${this.selected}`)
          .getComponent(cc.Animation)
          .play("idle");
      }
      this.selected = i;
      this.node
        .getChildByName(`Box${i}`)
        .getComponent(cc.Animation)
        .play("roll");
    };
    this.node.getChildByName("DoBuy").on(Input.EventType.TOUCH_END, (_) => {
      ...
      if (this.selected !== -1) {
        this.node
          ...
          .play("open");
        this.client
          .open_box(this.boxPerosnalItems[this.selected].outpoint)
          .then((resp) => {
            console.log("tx hash: ", resp);
            const msgBoxNode = cc.instantiate(this.msgBoxPrefab);
            msgBoxNode.name = `MsgBox`;
            ...
            msgBoxNode
              ...
              .getComponent(
                cc.Label
              ).string = `Do Open Box ${this.selected} success hash:\n${resp}`;
            msgBoxNode.setPosition(0, 500);
            cc.tween(msgBoxNode)
              .to(
                0.5,
