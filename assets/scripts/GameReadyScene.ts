...
export class GameReadyManage extends Component {
  ...
  doBattle() {
    ...
    for (this.round = 0; this.round < 5; this.round++) {
      ...
      const tikResult = `round:\t${this.round + 1}\t, using:\t${tikC.skill}`;
      ...
      tikItem.getComponent(cc.Label).horizontalAlign =
        cc.HorizontalTextAlignment.LEFT;
      tikItem.getComponent(cc.Label).verticalAlign =
        cc.VerticalTextAlignment.TOP;
      tikItem.getComponent(cc.Label).string = `Tik: ${tikResult}`;
      tikItem.getComponent(cc.Label).overflow = cc.Overflow.RESIZE_HEIGHT;
      tikItem.getComponent(cc.UITransform).setContentSize(300, 25);
      ...
      const tokResult = `round:\t${this.round + 1}\t, belong to:\t${
        tokC.tribe
      }`;
      ...
      tokItem.getComponent(cc.Label).horizontalAlign =
        cc.HorizontalTextAlignment.LEFT;
      tokItem.getComponent(cc.Label).verticalAlign =
        cc.VerticalTextAlignment.TOP;
      tokItem.getComponent(cc.Label).string = `Tok: ${tokResult}`;
      tokItem.getComponent(cc.Label).overflow = cc.Overflow.RESIZE_HEIGHT;
      tokItem.getComponent(cc.UITransform).setContentSize(220, 25);
