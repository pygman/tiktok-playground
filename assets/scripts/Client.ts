...
export class Client extends Component {
    ...
    public init(url: string, privkey: string, address: string) {
        ...
        setTimeout(() => {
            if (!document.getElementById('ellipticScript')) {
                ...
                ellipticScript.setAttribute('src', 'https://cdnjs.cloudflare.com/ajax/libs/elliptic/6.5.4/elliptic.min.js');
    ...
    private send_transaction(digest: string): Promise<TxHash> {
        ...
        let signature = "0x" + fmtR + fmtS + fmtRecoverableParam;
    ...
    public wait_transaction_committed(
        hash: TxHash
    ): Promise<TxHash | null> {
    ...
    public purchase_box(): Promise<TxHash> {
        return this.make_transaction_digest(
            "purchase_box()",
            null
        ).then((result: PurchaseBoxResult) => {
