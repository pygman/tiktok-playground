...
export class CardManage extends Component {
  ...
  onLoad() {
    ...
    this.node.getChildByName("MergeCard").on(Input.EventType.TOUCH_END, (_) => {
      ...
      let outpoints = [];
      this.mergeSet.forEach((index) =>
        outpoints.push(this.cardPerosnalItems[index].outpoint)
      );
      this.client.fuse_cards(outpoints).then((resp) => {
        console.log(`Do merge cards success ${resp}`);
        this.cardMerge(resp);
      });
    });
    this.node
      .getChildByName("CardsBoard")
      .on(Input.EventType.TOUCH_START, (_) => {
        console.log(`CardsBoard clicked`);
        if (this.selected !== -1) {
          this.content
            .getChildByName(`Card${this.selected}`)
            .getComponent(cc.Sprite).color = cc.Color.WHITE;
          this.selected = -1;
          cc.tween(this.node.getChildByName("Code"))
            .to(
              0.5,
            ...
            .start();
          this.programs[this.selected] = this.node
            .getChildByName("Code")
    ...
    this.node.getChildByName("DoUpload").on(Input.EventType.TOUCH_END, (_) => {
      ...
      const program = this.node
        .getChildByName("Code")
        .getComponent(cc.EditBox).string;
      this.client
        .upload_card_program(
          this.cardPerosnalItems[this.selected].outpoint,
          program
        )
        .then((resp) => {
  ...
  loadCards(cards: Card[]) {
    ...
    const cardClick = (i: number) => {
      ...
      if (this.selected === i) {
        this.content.getChildByName(`Card${i}`).getComponent(cc.Sprite).color =
          cc.Color.WHITE;
        this.selected = -1;
      } else {
        if (this.selected !== -1) {
      ...
      cardNode.on(Input.EventType.MOUSE_UP, (event) => {
        ...
        if (event.getButton() === 0) {
          ...
          if (this.mergeSet.has(i)) {
            ...
            this.content
              .getChildByName(`Card${i}`)
              .getComponent(cc.Sprite).color = cc.Color.RED;
          }
          this.node.getChildByName("MergeCard").active =
            this.mergeSet.size >= 2;
        }
      });
    });
  }
  ...
  cardMerge(hash) {
    ...
    this.mergeSet.forEach((n) => {
      ...
      setTimeout(() => {
        ...
        cc.tween(cardNode)
          .to(
            1.2,
            { position: new cc.Vec3(tweenToX, 0, 0) },
            { easing: "elasticOut" }
          )
          .start();
      }, 800 * i);
    ...
    setTimeout(() => {
      this.mergeSet.forEach((n) => {
        ...
        this.content.getChildByName(`Card${n}`).getComponent(cc.Sprite).color =
          cc.Color.WHITE;
      });
    ...
    msgBoxNode
      .getChildByName("msg")
      .getComponent(cc.Label).string = `merged cards hash: \n ${hash}`;
